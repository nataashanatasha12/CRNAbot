...
async def lookup(
    ...
    async def _load_girbo_org():
        if not want_reporting and not want_egrul:
            return None
        try:
            return await asyncio.wait_for(get_organization(inn), timeout=35.0)
    ...
    async def _load_rmsp():
        try:
            return await asyncio.wait_for(lookup_rmsp(inn), timeout=18.0)
        except asyncio.TimeoutError:
            return TimeoutError("Реестр МСП: таймаут")

    async def _load_bankruptcy():
        try:
            return await asyncio.wait_for(lookup_bankruptcy(inn), timeout=12.0)
        except asyncio.TimeoutError:
            return TimeoutError("Банкротство: таймаут")
    ...
    try:
        result["description"] = await asyncio.wait_for(
            build_company_description(
                short_name=short_name,
                city=city,
                name=egrul_name or girbo_name,
