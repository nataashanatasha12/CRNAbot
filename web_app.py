...
from services.egrul import EgrulError, EgrulRecord, download_egrul_pdf, search_egrul
from services.egrul_parse import parse_egrul_pdf
from services.bankruptcy import BANKRUPTCY_URL, STATUS_UNKNOWN, BankruptcyCheck, lookup_bankruptcy
...
from services.rmsp import RmspError, RmspRecord, download_rmsp_report, lookup_rmsp
...
async def _resolve_download_names(
    ...
    *,
    ...
    if not ogrn or (not egrul_name and org is None):
        try:
            rows = await search_egrul(inn)
            if rows:
                ogrn = ogrn or rows[0].ogrn or ""
                egrul_name = egrul_name or rows[0].name
        except (EgrulError, aiohttp.ClientError, asyncio.TimeoutError):
            pass
...
async def lookup(
    ...
    # Карточка: без Федресурса (иначе elif всё равно ходит на fedresurs.ru ~30 с).
    _lookup_skip_fedresurs = True
    ...
    async def _load_egrul_search() -> EgrulRecord | Exception | None:
        if not want_egrul:
            return None
        try:
            rows = await asyncio.wait_for(search_egrul(inn), timeout=28.0)
            return rows[0] if rows else None
        except Exception as exc:
            logger.info("ЕГРЮЛ search lookup inn=%s: %s", inn, exc)
            return exc
    ...
    egrul_search_raw, girbo_result, rmsp_result, bankruptcy_result = await asyncio.gather(
        _load_egrul_search(),
        _load_girbo_org(),
        _load_rmsp(),
        _load_bankruptcy(),
    ...
    if isinstance(egrul_search_raw, EgrulRecord):
        r = egrul_search_raw
        egrul_name = r.name
    ...
    elif isinstance(egrul_search_raw, asyncio.TimeoutError):
        result["errors"].append("ЕГРЮЛ: сервис слишком долго отвечает (повторите поиск)")
    elif isinstance(egrul_search_raw, aiohttp.ClientError):
        result["errors"].append(f"ЕГРЮЛ: не удалось подключиться ({egrul_search_raw})")
    elif isinstance(egrul_search_raw, Exception):
        if isinstance(egrul_search_raw, EgrulError):
            result["errors"].append(f"ЕГРЮЛ: {egrul_search_raw}")
        else:
            result["errors"].append(f"ЕГРЮЛ: {egrul_search_raw}")
    ...
    if (
        not _lookup_skip_fedresurs
        and want_reporting
        and not _has_girbo_audit_for_year(girbo_documents, report_year)
    ):
    ...
    elif want_reporting and not _lookup_skip_fedresurs:
        try:
    ...
    if not result.get("egrul") and want_egrul and girbo_name and (girbo_result if not isinstance(girbo_result, Exception) else None):
        org_for_egrul = girbo_result
        ogrn_fb = (org_for_egrul.ogrn or "").strip()
        if ogrn_fb:
            result["egrul"] = {
                "name": girbo_name,
                "inn": inn,
            ...
            egrul_name = egrul_name or girbo_name
