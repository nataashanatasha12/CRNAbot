...
async def _enrich_org_okved_from_card_page(
    ...
    *,
    ...
    if not skip_playwright and _okved_specificity(best)[0] < 2:
        try:
...
async def get_organization(inn: str, *, skip_playwright_okved: bool = False) -> BoOrganization:
    inn = normalize_inn(inn.strip())
    if not inn.isdigit() or len(inn) not in (10, 12):
    ...
    async with aiohttp.ClientSession(
        ...
        org_okved = await _enrich_org_okved_from_card_page(
            session, org_id, GIRBO_OKVED_PERIOD, org_okved, skip_playwright=skip_playwright_okved
        )
